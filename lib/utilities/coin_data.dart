...
class CoinData {
 ...
 Future getCoinData(String currency) async{
    ...
    for(var a in cryptoList){
      http.Response response = await http.get(Uri.parse("$kBaseURL/${a.split("/").first}/$currency?apikey=$kAPIKey"));
       if(response.statusCode == 200) {
        var result = jsonDecode(response.body);
        // print(result);
        CoinModel coinModel = CoinModel(icon: a.split("/").first, name: a.split("/").last, price: result['rate']);
        cryptoPrices.add(coinModel);
       }else{
        print(response.statusCode);
       }
    }
    // print(cryptoPrices.length);
    return cryptoPrices;
